...
#    BCE = tf.reduce_sum(softmax_cross_entropy_with_logits(logits=x_hat, labels=x), reduction_indices=1)
...
#    loss_summ = tf.summary.scalar("lowerbound", loss)
    train_step = tf.train.AdadeltaOptimizer(learning_rate).minimize(regularized_loss)
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            for j in range(total):
                _, cur_loss,cur_kld = sess.run([train_step, loss,kld], feed_dict={x: mnist.next_batch(batch_size)})
                if j % 5 == 0:
